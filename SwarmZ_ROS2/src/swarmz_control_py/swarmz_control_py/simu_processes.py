...
def main():
    ...
    for i in range(0,node.drones):
        commands+=("cd "+node.px4+" && export PX4_HOME_LAT="+str(node.latitude)+" && export PX4_HOME_LON="+str(node.longitude)+" && export PX4_HOME_ALT="+str(node.altitude)+" && PX4_SYS_AUTOSTART=4001 PX4_GZ_MODEL_POSE='"+str(i*5)+",0' PX4_GZ_MODEL=x500 ./build/px4_sitl_default/bin/px4 -i "+str(i+1),)
